...
public abstract class VerifyUtil {
	...
	public static final void verifySameSize(Matrix... matrices) {
		...
		for (int i = matrices.length; --i != 0;) {
			verifyTrue(Arrays.equals(matrices[i].getSize(), matrices[i - 1].getSize()),
					"matrices have different sizes");
		}
	}

	public static final void verifySameSize(double[][] source1, double[][] source2,
			double[][] target) {
		...
		verifyEquals(source1[0].length, source2[0].length,
				"matrix1 and matrix2 have different sizes");
		verifyEquals(source2[0].length, target[0].length,
