...
public abstract class AbstractByteBufferConcatenation implements ByteBufferConcatenation {
	...
	public static final int COPYBUFFERSIZE = 1048576;

	protected ByteBuffer[] byteBuffers;
	protected final long totalLength;

	public AbstractByteBufferConcatenation(final ByteBuffer... byteBuffers) {
	...
	public final void deleteBytes(final long position, final int count) {
		synchronized (this) {
			final byte[] copyBuffer = new byte[COPYBUFFERSIZE];
			for (long pos = position; pos < totalLength; pos += COPYBUFFERSIZE) {
				final int length = (int) Math.min(COPYBUFFERSIZE, totalLength - pos - count);
	...
	public final void getBytes(final byte[] bytes, final long position, final int length) {
		...
		int pos = 0;
		while (remaining > 0) {
			final int byteCount = Math.min((int) (byteBuffers[byteBufferId].capacity() - offset), remaining);
