...
import org.ujmp.core.calculation.Gemm;
...
public class Mtimes extends AbstractDoubleCalculation {
	...
	public Matrix calcDoubleMatrix2D(final DoubleMatrix2D m1, final DoubleMatrix2D m2) {
	...
	public Matrix calc(final Matrix m1, final Matrix m2) {
		if (m1 instanceof DoubleMatrix2D && m2 instanceof DoubleMatrix2D) {
			return calcDoubleMatrix2D((DoubleMatrix2D) m1, (DoubleMatrix2D) m2);
		} else {
			Matrix C = MatrixFactory.dense(m1.getRowCount(), m2.getColumnCount());
			Gemm.INSTANCE.calc(1.0, m1, 1.0, m2, C);
			return C;
		}
	}
}
