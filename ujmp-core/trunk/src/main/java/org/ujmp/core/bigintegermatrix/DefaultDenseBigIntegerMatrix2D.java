...
public class DefaultDenseBigIntegerMatrix2D extends AbstractDenseBigIntegerMatrix2D {
	...
	public DefaultDenseBigIntegerMatrix2D(BigInteger[]... v) {
		this.values = v;
	}
	...
	public final Matrix transpose() {
		BigInteger[][] result = new BigInteger[values[0].length][values.length];
		for (int r = result.length; --r >= 0;) {
			for (int c = result[0].length; --c >= 0;) {
				result[r][c] = values[c][r];
			}
		}
		return new DefaultDenseBigIntegerMatrix2D(result);
	}
