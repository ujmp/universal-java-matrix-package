...
public class LuceneMap<K, V> extends AbstractMap<K, V> implements Flushable, Closeable, Erasable {
    ...
    public synchronized ObjectMatrix2D search(String searchString) {
        try {
            MultiFieldQueryParser p = new MultiFieldQueryParser(new String[]{VALUESTRING},
                    getAnalyzer());
            ...
            if (indexWriter == null) {
                if (DirectoryReader.indexExists(getDirectory())) {
                    if (!readOnly) {
                        ...
                        //}
                        indexWriter = new IndexWriter(getDirectory(), new IndexWriterConfig(Version.LATEST, getAnalyzer()));
                    }
                } else {
                    if (!readOnly) {
                        indexWriter = new IndexWriter(getDirectory(), new IndexWriterConfig(Version.LATEST, getAnalyzer()));
